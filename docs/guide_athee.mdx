...
-	L’Athée est considéré comme maléfique et comme Sbire ou Démon pour la <Role VOYANTE />, l’<Role ENQUETEUR />, le <Role CORNEILLE />, l’<Role EMPATHE />, le <Role MERCENAIRE />, la <Role PUCELLE />. L’Athée n’est pas considéré comme gentil ou comme Villageois ou Marginal pour la <Role LAVANDIERE /> ou l’<Role ARCHIVISTE />.
...
-	Mettez deux <Role EMPATHE />s. Ou deux <Role CORNEILLE />s. Ou deux <Role SAINT />. Ou deux <Role RECLUSE />. Assurez-vous d’avoir un seul doublon (le Démon ne se mettra pas en doublon normalement) Faites en sorte que le doublon soit sur un rôle que les maléfiques bluffent souvent (comme <Role LAVANDIERE /> ou <Role FOSSOYEUR />, par exemple). Pour ajoutez du fun, mettez deux <Role PUCELLE />s en jeu mais une seule qui fonctionnera. Ou deux <Role MERCENAIRE />s mais un seul qui peut tuer.
