...
> Le Chapelier meurt. L’<Role OJO /> choisit de devenir le <Role NO_DASHII /> et l’<Role AVOCAT_DU_DIABLE /> choisit de devenir la <Role CROQUEUSE_D_HOMMES />.

> Le <Role VIGORMORTIS /> a tué sa <Role VIEILLE_CHOUETTE /> puis, plus tard, tue le Chapelier. Les deux joueurs choisissent de garder leur rôle respectif donc les effets du <Role VIGORMORTIS /> ne sont pas perdus.

> L’<Role ASSASSIN /> tue un joueur. Le Chapelier est exécuté. Cette nuit, l’<Role ASSASSIN /> devient le <Role CONSPIRATEUR />. Le <Role CONSPIRATEUR /> fait non de la tête pour rester <Role CONSPIRATEUR />. Le Conteur fait non de la tête car le <Role CONSPIRATEUR /> a déjà été choisi et incite le <Role CONSPIRATEUR /> a choisir à nouveau donc il choisit <Role ASSASSIN />. Le nouvel <Role ASSASSIN /> utilise ensuite sa nouvelle capacité pour tuer un joueur.
...
- <Role EMEUTIER /> : Jinx – Si le Chapelier meurt et qu’un Emeutier choisit un autre type de Démon, une équipe de maléfique “normale” est créé à partir des joueurs Emeutiers. Si le Chapelier meut et que le Démon choisit Emeutier, les Sbires deviennent Emeutiers aussi.
