...
- Si vous arrivez à vous coordonner avec un <Role.CROQUEMORT />, vous pouvez exécuter vos voisins pour avoir des informations sur les personnes suivantes. De son côté le <Role.CROQUEMORT /> pourra confirmer les rôles des personnes exécutées. C’est un combo très puissant.
...
- Faites attention à l'<Role.TRAITRE /> ! Il peut être vu comme Gentil et du coup biaiser l’information que vous avez reçue. De même, le <Role.ERMITE /> peut être vu Maléfique, vous penserez alors être assis d’un Maléfique, à vous de chercher les alignements.
