...
> Lors de la troisième nuit, le <Role FAUSSAIRE /> choisit le rôle de l'Artiste et récupère ses capacités. Si un Artiste est en jeu, il deviendra ivre et perdra son pouvoir. Il peut à ce moment y avoir deux artistes qui peuvent maintenant utiliser leur capacité.
Si le <Role FAUSSAIRE /> meurt ou devient un autre rôle pour une quelconque raison, le premier Artiste redevient alors sobre et récupère ses capacités.  

> Si je choisis le rôle du <Role CRIEUR />, qu’un <Role CRIEUR /> est déjà en jeu mais mort, alors il n'y aura pas d'impact sur le joueur ayant le rôle de <Role CRIEUR />.
