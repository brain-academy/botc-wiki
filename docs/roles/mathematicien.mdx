...
Chaque nuit, vous apprenez combien de joueurs ont eu leur capacité altérée (depuis l’aube) à cause de la capacité d’un autre joueur.

Le Mathématicien sait combien de choses se sont « mal passés » depuis l’aube.

-	Quand une capacité ne fonctionne pas comme prévu dû à une interférence d’un autre rôle, le Mathématicien saura que ça s’est produit. Il apprendra que quelque chose se passe mal si une information est fausse alors qu’elle devait être vraie ou si une capacité aurait dû marcher mais n’a pas fonctionné à cause d’un autre rôle.  
...
-	Le Mathématicien ne détecte pas l’ivresse ou l’empoisonnement en soi mais détecte quand ces capacités des joueurs ivres ou empoisonnés n’ont pas fonctionné comme prévu. La Recluse considérée comme maléfique pour un Cuistot ou un Soldat empoisonné qui meurt du Démon seront détectés par le Mathématicien. L’Empathe empoisonné qui reçoit une bonne info ne le serait pas. 

### Exemples  

> L’<Role ORACLE /> empoisonné apprend que deux joueurs morts sont maléfiques alors qu’il y en a en réalité 3. Toutes les autres capacités ont fonctionné normalement. Plus tard dans la nuit, le Mathématicien a un 1.

> Le <Role CHARMEUR_DE_SERPENT /> empoisonné choisit un Villageois et rien ne se passe. Le <Role JONGLEUR /> ivre, reçoit une information correcte. Le <Role SAVANT /> reçoit deux informations correctes. Plus tard dans la nuit, le Mathématicien a un 1 car la capacité du <Role CHARMEUR_DE_SERPENT /> et du <Role JONGLEUR /> ont fonctionné normalement alors qu’une des informations du <Role SAVANT /> aurait dû être fausse au lieu de vraie.

> Un <Role VORTOX /> est en jeu. Cinq Villageois ont eu de fausses informations. La <Role SORCIERE /> est ivre et lorsque le joueur maudit nomine, rien ne se passe. Bien que 6 capacités au total n’aient pas fonctionné, le Mathématicien a un 4 à cause de la capacité du <Role VORTOX />.
...
- Prêtez attention aux changements du jeu qui affecteront votre information et soyez vigilant si elle change brutalement. Si un joueur exécuté prétend être la <Role DULCINEE />, votre nombre pourrait augmenter cette nuit. D’une autre manière, si vous obtenez subitement un « 2 » alors que vous aviez « 0 » auparavant, vous pourriez suspecter que la <Role GUENAUDE /> a créé un <Role NO_DASHII />.

- Vérifiez avec votre Conteur quelle sorte d’altération de capacité vous détectez. Dans Sects & Violets, vous détectez typiquement quand les Villageois ont leur capacité altérée à cause d’une capacité de maléfique comme le <Role NO_DASHII /> ou le <Role VORTOX />. Cependant, vous allez aussi détectez des capacités alétérées causées par la mort de la <Role DULCINEE /> ou le <Role PHILOSOPHE /> copiant un rôle déjà en jeu ou un <Role CHARMEUR_DE_SERPENT /> empoisonné qui choisit le Démon mais qu’il ne se passe rien.
...
- Changez drastiquement votre information quand un changement évident dans le jeu devrait le provoquer. Par exemple, si une <Role DULCINEE /> meurt ou qu’un <Role PHILOSOPHE /> choisit un rôle déjà en jeu, changez votre information en conséquence indiquera bien que vous êtes le Mathématicien et que la <Role DULCINEE /> et le <Role PHILOSOPHE /> disent bien la vérité.

- Si vous savez quel type de Démon est en jeu, donnez une information cohérente avec le Démon. Cela vous fera paraître gentil. Par exemple, si vous savez qu’un <Role VORTOX /> est en jeu (et vous pensez que le village le sait ou va le savoir), alors donner une information évidemment fausse semble raisonnable. Ou si vous savez qu’un <Role FANG_GU /> est en jeu (et vous pensez que le village le sait ou vas le savoir) alors donner une information qui implique que les capacités fonctionnent normalement.

- Si vous savez quel type de Démon est en jeu mais que vous ne croyez pas que le village va le trouver, alors donnez une information contradictoire avec le type réel du Démon. Si vous arrivez à faire croire au village qu’un Démon différent est en jeu, alors ils seront très confus. Par exemple, si un <Role VIGORMORTIS /> est en jeu, vous pouvez continuer à donner un «1» ou un «2» donnant ainsi l’impression que c’est le <Role NO_DASHII /> qui est en jeu.
