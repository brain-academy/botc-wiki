...
> Le Mezepheles dit en privé au <Role NOBLE /> le mot secret « Constantinople ». Le <Role NOBLE /> va voir le Conteur en privé et dit « Constantinople ». Cette nuit, la <Role COURTISAN /> rend le Mezepheles ivre. Le <Role NOBLE /> reste gentil. 
...
- Choisissez un joueur gentil qui a plus de raisons de passer maléfique. Une <Role VOYANTE /> ou un <Role CORNEILLE /> qui ne pense pas avoir de bonnes informations peut penser que les maléfiques gagnent et vont probablement accepter de passer maléfique. Quelques Marginaux comme la <Role DULCINEE /> ou le <Role MALADROIT />, pourrait être inquiets des dégâts de leur capacité sur le village et donc accepter de passer maléfique. Si vous pouvez avoir une conversation privée avec un de ces joueurs, dites-lui simplement le mot secret et que vous êtes Mezepheles. Si vous avez de la chance et que vous avez approché la bonne personne, elle pourrait dire le mot secret.
...
- Rôles maléfiques : Le joueur qui passe maléfique conserve son rôle et son type quel qu’il soit donc il peut toujours être montré à l’Aéronaute comme étant de ce type ou une Lavandière ou un Archiviste comme ce rôle, le Jongleur peut avoir un nombre s’il jongle son rôle, le Parieur peut parier son rôle et ne pas mourir, le <Role FOSSOYEUR /> verra son jeton de rôle s’il est exécuté, etc…

- <Role CANNIBALE /> : Si le joueur qui est passé maléfique est exécuté, le Cannibale serait empoisonné. Le fait qu’il soit maléfique est le déclencheur, pas le fait d’avoir un rôle de Villageois ou de Marginal.
...
- <Role IMMACULEE /> : Un Villageois maléfique peut nomine l'Immaculée et être exécuté ! De la même manière, le Mezepheles peut faire passer l'Immaculée maléfique !  

- <Role REVEUR /> : Le Rêveur doit toujours voir un jeton de rôle de gentil (Villageois ou Marginal) pour un jeton de rôle de maléfique (Sbire ou Démon) indépendamment de l’alignement du joueur à ce moment-là. Donc si le Rêveur choisit le Villageois passé maléfique par le Mezepheles, il verra toujours le jeton de rôle de ce Villageois et un jeton de rôle de Sbire ou de Démon même si celui-ci est maléfique.
...
- <Role BRUTE /> : Si une Brute dit le mot du Mezepheles durant la journée, il passe maléfique la nuit indépendamment de sa capacité. Le Mezepheles n’est pas ivre car le Mezepheles ne choisit pas activement le joueur qui est Brute avec sa capacité. C’est la Brute qui a activement déclenché la capacité du Mezepheles. Donc une Brute peut passer maléfique par le Mezepheles et ensuite être choisi par un joueur gentil la même nuit et donc changer d’alignement deux fois dans la même nuit.

- <Role RECLUSE /> / <Role ESPIONNE /> : Si une Recluse dit le mot du Mezepheles, le Conteur peut choisir de la considérer maléfique à ce moment-là et ne pas déclencher la capacité du Mezepheles. Dans ce cas, ça serait le prochain joueur gentil à dire le mot qui passera maléfique la nuit suivante. De la même manière, et si le Conteur est vilain, il peut considérer l’Espionne qui dit le mot comme gentille ce qui déclenche la capacité du Mezepheles qui ne pourra plus être utilisée sur un autre joueur.

- <Role JUMELLE_MALEFIQUE /> : Si joueur qui passe maléfique était la jumelle gentille, la Jumelle maléfique doit immédiatement être réveillé avec un nouveau joueur gentil qui sera son nouveau jumeau. La jumelle précédent n’est plus une jumelle valide.
...
- <Role TIMONSTRE /> : Un Mezepheles qui a le p’tit monstre ne peut pas faire passer maléfique un joueur protégé par le Moine ou un Soldat.
...
- <Loric CHASSEUR_D_ORAGES /> :  Si le joueur favorisé par le Chasseur d’orages passe maléfique, il ne peut toujours mourir que par exécution.

- <Fabled TOUR_D_IVOIRE /> : Empêchera un joueur gentil de passer maléfique après avoir dit le mot s’il y a déjà un extra maléfique. La capacité du Mezepheles est alors perdue dans ce cas.