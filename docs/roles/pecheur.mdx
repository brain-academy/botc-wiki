...
> Le Pêcheur apprend "3"Garde les joueurs qui prétendent être Marginal en vie." Ces joueurs sont secrètement le <Role MALADROIT /> et le <Role TOURMENTEUR />. Le Conteur pense que garder ces joueurs en vie va plus probablement se terminer avec une victoire du Village exécutant le Démon.
...
- Les rôles qui peuvent être considéré d’alignement différents peuvent affecter le conseil donné au Pêcheur. Par exemple, un <Role RECLUSE /> peut être considéré comme le Démon ce qui pourrait inciter le Conteur à vous donner un conseil pour l’exécuter. Attention à cette possibilité.
...
- Donnez un faux conseil qui incrimine un joueur gentil. Utilisez le pour jeter de la suspicion sur les informations d’un <Role FOSSOYEUR />, d’une <Role VOYANTE /> ou pour faire exécuter un <Role SAINT />.
