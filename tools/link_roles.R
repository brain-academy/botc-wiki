...
# Load Tags File.
list_tags <- readLines(con = "../src/components/role/role.tsx",
                       encoding = "UTF-8")
# Extract Tags.
tags_output <- list_tags %>%
  .[stringr::str_detect(., "static readonly")] %>%
  trimws() %>%
  stringr::str_replace("^static readonly ", "") %>%
...
plyr::l_ply(
  list_files,
  function(role_file) {
    # Get the path of the role file.
    role_file_path <- paste0("../docs/roles/", role_file)
    # Open the role file.
    role_text <- readLines(con = role_file_path,
                           encoding = "UTF-8")
